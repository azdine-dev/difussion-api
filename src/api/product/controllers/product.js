...
module.exports = createCoreController("api::product.product", ({ strapi }) => ({
  ...
  async count(ctx) {
    ...
    return strapi.db.query("api::product.product").count({ where: query });
  },
  async findElementsProducts(ctx) {
    let entities;
    const result = await strapi.db.query("api::product.product").findMany({
      where: {
        vendor: {
          $contains: "elements",
        },
  ...
  async findBySlug(ctx) {
    ...
    const { demo, isQuickView } = ctx.query;
    let categories,
      filteredProductIds = [];
    ...
    for (let i = 0; i < category.length; i++) {
      ...
      // })
      // .fetch();
      let ids = categories?.products.reduce((acc, cur) => {
        return [...acc, cur.id];
      }, []);
    ...
    let relatedProducts =
      relatedEntities.length > 0
        ? relatedEntities.filter((product) => {
            return (
              product.vendor.split(",").indexOf("elements") > -1 ||
              product.vendor.split(",").indexOf(demo) > -1
            );
          })
        : [];
    ...
    if (curIndex >= 1) prevProduct = relatedProducts[curIndex - 1];
  ...
  async findShopProducts(ctx) {
    ...
    return {
      ...
      products: filteredProducts.slice((page - 1) * perPage, page * perPage),
    };
  },
...
//       .query("product")
//       .model.query((qb) => {
//         qb.where("vendor", "LIKE", "%" + demo).orWhere(
//           "vendor",
//           "LIKE",
//           "%" + demo + ",%"
//         );
...
//       .query("blog")
//       .model.query((qb) => {
//         qb.where("demo", "LIKE", "%" + demo).orWhere(
//           "demo",
//           "LIKE",
//           "%" + demo + ",%"
//         );
...
//     const relatedEntities = await strapi
//       .query("product")
//       .model.query((qb) => {
//         qb.select(
//           "id",
